...
def main():
  ...
  k = 0
  score = 0
  with open('../data/1112model_shuffle.ndjson') as f:
    data = ndjson.load(f)
  ...
  while True:
    ...
    data, command = df_shinoda["events"][k], None
    print(data)
    if command is not None:
      print('command: %s' % command)
      continue
    if k == df_shinoda.shape[0] - 1:
      break
    if data is None:
      print('done.')
      break
    ...
    print("本当は{}".format(df_shinoda["kana"][k]))
    #time.sleep(1)

    if df_shinoda["kana"][k] == most_likely_result[0]:
      score += 1
      print("ピンポン")
    k += 1
  print("精度は{}%".format(score/k*100))
